...
- 성공 매트릭스(모드·CLI·OS별 실제 관측 결과)를 `docs/specs/appendix/cli-matrix.md`로 기록 — 이후 Phase의 플래그 변경 근거.
...
**Phase R3 — 레거시 이동 + 디자인 시스템 초기** (plan: `docs/plans/2026-04-20-rolestra-phase-r3.md`, done-checklist: `docs/checklists/r3-done-checklist.md`)
...
- [x] Legacy channel warning 현황 문서화 + 격리 테스트(제거는 R11) — 산출물: `docs/specs/appendix/legacy-channels.md`, `src/renderer/__tests__/legacy-channel-isolation.test.ts`
- [x] 6 테마 × 2 모드 스크린샷 증빙 — 산출물: `docs/specs/appendix/r3-evidence/{warm,tactical,retro}-{light,dark}.png` + README. 실제 Electron 런타임 캡처 완료 (2026-04-20).
- [x] typecheck/lint/i18n:check/theme:check/build exit 0 + test 64 pre-existing fail 유지(R3 신규 30+ 테스트 전부 green) — 산출물: `docs/checklists/r3-done-checklist.md`
...
**Phase R4 — 대시보드 + 프로젝트 관리** (plan: `docs/plans/2026-04-20-rolestra-phase-r4.md`, done-checklist: `docs/checklists/r4-done-checklist.md`)
...
- [ ] 6 테마 대시보드 스크린샷 증빙 + E2E 캡처 — 산출물: `docs/specs/appendix/r4-evidence/*.png` **(수동 캡처 대기 — WSL 런타임 제약 동일. R4 code correctness는 non-blocking, 시각 sign-off는 Windows/native에서)**
- [x] typecheck/lint/test/i18n:check/theme:check/build exit 0 + R4 신규 테스트 전부 green + Playwright 로컬 pass — 산출물: `docs/checklists/r4-done-checklist.md` (Playwright 로컬 pass는 위 항목과 동일 이연)
...
**Phase R5 — 채널 + 메신저 본체** (plan: `docs/plans/2026-04-21-rolestra-phase-r5.md`, prep: `docs/reports/analysis/r5-prep-messenger-theme-analysis.md`, done-checklist: `docs/checklists/r5-done-checklist.md`)
...
- [x] typecheck/lint/test/i18n:check/theme:check/build exit 0 + R5 신규 테스트 green + done-checklist 작성 — 산출물: `docs/checklists/r5-done-checklist.md` (Playwright 로컬 pass는 위 항목과 동일 이연)
...
**Phase R6 — 회의(SSM) 연동 + v2 engine 잔재 완전 청산** (plan: `docs/plans/2026-04-22-rolestra-phase-r6.md`, done-checklist: `docs/checklists/r6-done-checklist.md`)
...
- [x] typecheck(전체) / typecheck:web / lint / test / i18n:check / theme:check / build exit 0 + R6 신규 테스트 green + done-checklist 작성 — 산출물: `docs/checklists/r6-done-checklist.md`
...
**Phase R7 — 승인 시스템(ApprovalInbox + CLI permission adapter v3 전면 교체)** (plan: `docs/plans/2026-04-22-rolestra-phase-r7.md`, done-checklist: `docs/checklists/r7-done-checklist.md`)
...
- [x] §7.8 NotificationService approval_pending 트리거 (비포커스 시 OS 알림 + 클릭 시 #승인-대기 라우팅 힌트) — 산출물: `src/main/approvals/approval-notification-bridge.ts` (ApprovalService 'created' → NotificationService.show(approval_pending), kind 별 title/body + 5s dedupe) + `src/main/approvals/__tests__/approval-notification-bridge.test.ts` (10 tests) + `src/shared/stream-events.ts` `stream:notification-clicked` 추가 + `src/main/streams/stream-bridge.ts` `services.notifications` connect → 'clicked' → `stream:notification-clicked` + `src/main/index.ts` wire
- [x] i18n populate `approval.kind.*` / `approval.systemMessage.*` / `messenger.approval.{reject,conditional}Dialog.*` / `messenger.approval.inbox.*` (Task 7 에서 populate) / `dashboard.approvals.*` (Task 4 범위 완료) + i18next-parser keepRemoved 확장 — 산출물: `src/renderer/i18n/locales/{ko,en}.json` + `i18next-parser.config.js`. `src/renderer/features/approvals/ApprovalInboxView.tsx` 에 `kindLabel(t, kind)` 정적 t() 앵커 추가로 `approval.*` top-level namespace orphan-prune 방지. `notification.approvalPending.*` 는 main-process 고정 라벨이므로 R10 renderer-side i18n migration 시점까지 deferred(§R8+ scope).
- [x] Playwright Electron E2E "승인 flow (CLI permission 또는 consensus decision happy path)" — 산출물: `e2e/approval-flow.spec.ts` (messenger → `#승인-대기` → ApprovalInboxView empty-state 내비게이션 검증. 전체 decide → minutes 포스팅 round-trip 은 R10 + CI matrix 때 AI 세션과 함께). **DONE_WITH_CONCERNS — WSL 런타임 제약(R4/R5/R6 과 동일). spec + fixture land 완료, CI 실행은 R10.**
- [x] typecheck(전체) / typecheck:web / lint / test / i18n:check / theme:check / build exit 0 + R7 신규 테스트 green + done-checklist 작성 — 산출물: `docs/checklists/r7-done-checklist.md` (14/14 task 맵 + 7 Known Concerns + D1~D8 Decision Log 요약. 게이트: typecheck/lint(0 errors)/i18n:check idempotent/theme:check clean/build success, approval+project+dashboard+meeting 도메인 257+ tests green. 레거시 v2 13 files 기존 failing 은 R11 legacy-cleanup 연기.)
...
**Phase R8 — 멤버 프로필 + 출근 상태** (plan: `docs/plans/2026-04-23-rolestra-phase-r8.md`, done-checklist: `docs/checklists/r8-done-checklist.md`)

구현 체크리스트 (완료 시 ✓ + 산출물 링크 채움):

- [x] §6 IPC 신규 `member:upload-avatar` (custom 아바타 파일 복사) + Shared `AvatarUploadRequest`/`Response` + zod — 산출물: `src/shared/{member-profile-types,ipc-types,ipc-schemas}.ts` + `src/preload/index.ts`
- [x] §7.1 `Avatar` 컴포넌트 (DEFAULT_AVATARS 8 emoji+color 렌더 + custom 분기 + initials fallback) + `WorkStatusDot` (4-state + i18n 라벨 + connecting pulse) + `ProfileAvatar` MemberView prop 위임 — 산출물: `src/renderer/components/members/{Avatar,WorkStatusDot}.tsx` + `src/renderer/components/shell/ProfileAvatar.tsx`
- [x] §7.1 `AvatarPicker` (8 default 갤러리 + custom upload 트리거 + 되돌리기) + `use-avatar-picker` hook — 산출물: `src/renderer/components/members/AvatarPicker.tsx` + `src/renderer/hooks/use-avatar-picker.ts`
- [x] §7.1 `MemberProfileEditModal` (Radix Dialog, role/personality/expertise/avatar 4 필드 + AvatarPicker + `member:update-profile` invoke) + `use-member-profile` hook — 산출물: `src/renderer/features/members/MemberProfileEditModal.tsx` + `src/renderer/hooks/use-member-profile.ts`
- [x] §7.1 `AvatarStore` Main (custom 파일 복사 to `<ArenaRoot>/avatars/<providerId>.<ext>` + ext 화이트리스트 png/jpg/jpeg/webp/gif + 5MB 제한) + `member:upload-avatar` handler + `ArenaRootService.getAvatarsDir()` — 산출물: `src/main/members/avatar-store.ts` + `src/main/arena/arena-root-service.ts` + `src/main/ipc/handlers/member-handler.ts` + `src/main/ipc/router.ts`
- [x] §7.1 `MemberProfilePopover` (Radix Popover, 헤더 Avatar+이름+role + 본문 personality/expertise/WorkStatusDot+라벨 + 푸터 4 액션: 편집/외근↔출근 토글/연락해보기/DM 시작) — 산출물: `src/renderer/features/members/MemberProfilePopover.tsx`
- [x] §7.1 아바타 클릭 wire 3 surface (메시지 버블 / MemberRow / PeopleWidget → 동일 Popover open → 'Popover 편집' → EditModal) — 산출물: `src/renderer/features/messenger/{Message,MemberRow}.tsx` + `src/renderer/features/dashboard/widgets/PeopleWidget.tsx`
- [x] §7.2 Production main/index.ts MemberProfileService boot (R2~R7 까지 테스트에서만 wire 되어 있던 6 IPC 의 production 활성화) + `MemberWarmupService.warmAll(providerIds)` (`Promise.allSettled` + per-provider 5초 timeout, fire-and-forget) — 산출물: `src/main/members/member-warmup-service.ts` + `src/main/index.ts` + `src/main/providers/registry.ts` 어댑터 검토
- [x] §7.2 `MeetingTurnExecutor` work-status 게이트 (`getWorkStatus !== 'online'` → skip + `meeting:turn-skipped` 이벤트 + system message append, SSM TURN_DONE/FAIL 발사 0) + Shared `meeting:turn-skipped` 이벤트 타입 + Renderer `use-meeting-stream` reducer + Thread 시스템 메시지 분기 — 산출물: `src/main/meetings/engine/meeting-turn-executor.ts` + `src/shared/meeting-stream-types.ts` + `src/main/streams/stream-bridge.ts` + `src/preload/index.ts` + `src/renderer/hooks/use-meeting-stream.ts` + `src/renderer/features/messenger/Thread.tsx`
- [x] §7.1 `MeetingTurnExecutor` v2 → v3 PersonaBuilder swap (v2 `engine/persona-builder.buildEffectivePersona(provider, opts)` 호출을 v3 `MemberProfileService.buildPersona(participantId)` + `buildPermissionRules` shim 으로 교체, v2 `engine/persona-builder.ts` 호출자 0 → R11 legacy 삭제 후보 등록) — 산출물: `src/main/meetings/engine/meeting-turn-executor.ts` + `src/main/engine/persona-builder.ts` (export 승격)
- [x] i18n populate `member.status.*` (4 라벨) / `member.avatarPicker.*` / `member.warmup.*` / `profile.editor.*` / `profile.popover.*` / `meeting.turnSkipped` + i18next-parser keepRemoved regex 확장. `notification.*` main-process 라벨은 R7 D 흐름과 동일하게 R10 deferred — 산출물: `src/renderer/i18n/locales/{ko,en}.json` + `i18next-parser.config.js`
- [x] §11 Playwright Electron E2E "메시지 버블 아바타 → popover → edit modal 저장 → 외근 토글 → 회의 턴 skip → 연락해보기 → 출근 복귀" — 산출물: `e2e/member-profile-flow.spec.ts` (mock provider + mock CLI 재사용. **WSL 제약 시 R4/R5/R6/R7 와 동일 DONE_WITH_CONCERNS 정책**)
- [x] typecheck(전체) / typecheck:web / lint / test / i18n:check / theme:check / build exit 0 + R8 신규 테스트 green + done-checklist 작성 — 산출물: `docs/checklists/r8-done-checklist.md`
...
**Phase R9 — 자율 모드 + 시스템 알림** (plan: `docs/plans/2026-04-23-rolestra-phase-r9.md`, done-checklist: `docs/checklists/r9-done-checklist.md`)

구현 체크리스트 (완료 시 ✓ + 산출물 링크 채움):

- [x] §6 IPC autonomy + queue + notification prefs 확정 — `project:set-autonomy(id, mode)` + `queue:list/add/reorder/remove/pause/resume` + `notification:get-prefs/update-prefs/test` + Shared autonomy/queue/notification types + zod — 산출물: `src/shared/{queue-types,notification-types,ipc-types,ipc-schemas}.ts` + `src/preload/index.ts`
- [x] §7.3 `AutonomyModeToggle` (프로젝트 헤더, manual/auto_toggle/queue 3분기) + 2단계 확인 다이얼로그 (manual → auto_toggle/queue 승격 시 Circuit Breaker 기본값 안내 + 명시적 재확인) + `use-autonomy-mode` hook + `stream:autonomy-mode-changed` reducer — 산출물: `src/renderer/features/projects/AutonomyModeToggle.tsx` + `src/renderer/features/projects/AutonomyConfirmDialog.tsx` + `src/renderer/hooks/use-autonomy-mode.ts`
- [x] §8 `QueuePanel` (프로젝트 상단 접기 가능 패널, 여러 줄 입력 → 항목 추가 / 목록 / drag 순서 변경 / 항목 제거 / pause / resume / 진행 포인터 표시) + `use-queue` hook + `stream:queue-updated` reducer — 산출물: `src/renderer/features/projects/QueuePanel.tsx` + `src/renderer/hooks/use-queue.ts`
- [x] §7.8 `NotificationPrefsView` (설정 UI — 4 kind `new_message`/`approval_pending`/`work_done`/`error` × 표시 on/off + 소리 on/off + 테스트 알림 버튼) + `use-notification-prefs` hook — 산출물: `src/renderer/features/settings/NotificationPrefsView.tsx` + `src/renderer/hooks/use-notification-prefs.ts` + 설정 surface 진입점
- [x] §8 `AutonomyGate` — `ApprovalService.create` 시 autonomyMode 분기 (manual=approval_item, auto_toggle/queue=정책 조건 accepted kind 만 auto accept + log + `#회의록` trace) — 산출물: `src/main/autonomy/autonomy-gate.ts` + `src/main/approvals/approval-service.ts` 후킹 + `src/main/autonomy/__tests__/autonomy-gate.test.ts`
- [x] §8 Circuit Breaker 실 이벤트 feed (4 tripwire: `files_per_turn`/`cumulative_cli_ms`/`queue_streak`/`same_error`) — `ExecutionService.dryRunApply` 직후 `recordFileChanges`, `CliRunner.spawn` 완료 시 `recordCliElapsed`, `QueueService.startNext` 시 `recordQueueStart`, `MeetingTurnExecutor`/`ApprovalService` 실패 시 `recordError` + `on('fired')` → `ProjectService.setAutonomy('manual')` + `approval_item` + `NotificationService.show('error', { reason })` — 산출물: `src/main/engine/v3-side-effects.ts` (breaker handler 완성) + `src/main/execution/execution-service.ts` + `src/main/providers/cli-process-manager.ts` + `src/main/queue/queue-service.ts` + `src/main/meetings/engine/meeting-turn-executor.ts` hooks
- [x] §8 Queue run loop — QueueService auto-start 다음 항목 (이전 meeting onFinal 직후) + 앱 재시작 복구 (`status='in_progress'` → `pending` 롤백 + `attempts` 유지) + pause/resume 정합성 + `stream:queue-updated` — 산출물: `src/main/queue/queue-service.ts` 확장 + `src/main/queue/__tests__/queue-service.test.ts` 확장 + `src/main/index.ts` recovery hook (meetingStarter production 주입은 R10 이월)
- [x] §8 `MeetingOrchestrator.onFinal` → `#회의록` 요약 메시지 자동 포스트 + `NotificationService.show('work_done', { projectId, meetingTitle })` (autonomyMode 무관 공통) + `#일반` 완료 알림(auto_toggle/queue 모드 한정, spec §8) — 산출물: `src/main/engine/v3-side-effects.ts` work-done handler 완성 + `src/main/meetings/engine/meeting-orchestrator.ts` wire
- [x] §7.8 Production wiring — `setNotificationServiceAccessor` + `setQueueServiceAccessor` + `streamBridge.connect({ ..., queue: queueService })` + `NotificationPrefsRepository` 기본값 seed + `notification-handler` / `queue-handler` production 활성화 (R8 Task 8 패턴 — 테스트에서만 wire 되어 있던 6 IPC 를 production 에서 throw 0 으로) — 산출물: `src/main/index.ts` R9 boot block
- [x] R8 인수인계 통합: (a) 외근 자동 timeout (1h 지나면 `offline-manual` → `online` 자동 복귀; 정책 값은 settings 에서 조정 가능; 기본 60분) + (b) `MemberWarmupService` backoff retry (실패 시 10s → 30s → 60s 지수, 최대 3회) — 산출물: `src/main/members/member-profile-service.ts` timeout 관리 + `src/main/members/member-warmup-service.ts` retry 확장 + tests
- [x] Stream 브로드캐스트 `stream:queue-updated` / `stream:notification-prefs-changed` / `stream:autonomy-mode-changed` — QueueService / NotificationService / ProjectService event emit → StreamBridge → preload 화이트리스트 → renderer hooks reducer — 산출물: `src/shared/stream-events.ts` + `src/main/streams/stream-bridge.ts` + `src/preload/index.ts` + `src/renderer/hooks/{use-queue,use-notification-prefs,use-autonomy-mode}.ts`
- [x] i18n populate `notification.*` (main-process 라벨 — R7/R8 에서 deferred 됐던 4 kind 고정 title/body + `notification.warmupFailed.*` 등) / `autonomy.*` (mode 3 라벨 + confirm dialog + Circuit Breaker 발동 안내) / `queue.*` (panel + toast + recovery banner) / `circuitBreaker.*` (4 tripwire 라벨 + reason 설명) / `settings.notifications.*` + i18next-parser keepRemoved 확장 — 산출물: `src/renderer/i18n/locales/{ko,en}.json` + `i18next-parser.config.js` + `src/main/notifications/notification-labels.ts` (D8 main-process dictionary)
- [x] §11 Playwright Electron E2E "auto_toggle 승격 2단계 확인 → 모드 전환 자동 accept → queue 추가/시작 → 완료 시 `#일반` 알림 + 다음 항목 자동 시작 → Circuit Breaker `files_per_turn > 20` 시 강제 manual 다운그레이드 + OS 알림" — 산출물: `e2e/autonomy-queue-flow.spec.ts` (Step A/B 활성, Step C R10 placeholder — mock breaker 주입 후 활성화. WSL 제약 시 R4~R8 와 동일 DONE_WITH_CONCERNS 정책)
- [x] typecheck(전체) / typecheck:web / lint / test / i18n:check / theme:check / build exit 0 + R9 신규 테스트 green + done-checklist 작성 — 산출물: `docs/checklists/r9-done-checklist.md`

**scope 경계 (R9에서 하지 않는 것, R10+ 이연):**
- DM read-receipt / typing indicator 실 이벤트 — R10
- FTS5 메시지 검색 — R10
- 설정 화면 "멤버 관리" 풀버전 / Provider 추가·삭제 UX / 설정 10 탭 재구성 — R10
- `stream:member-status-changed` 등 멤버 상태 실시간 브로드캐스트 — R10 (R9 의 autonomy / queue / notification stream 과 구조 동일하므로 R10 에서 같은 패턴 재사용)
- 낙관적 업데이트(optimistic UI) / Error Boundary 래핑 — R10
- 6 테마 autonomy/queue panel 시각 sign-off (Windows/native) — R11 릴리스 전
- Playwright CI integration + OS matrix — R10
- v2 engine 5 파일 물리 삭제 + v2 `engine/persona-builder.ts` — R11 legacy cleanup
- Retro 영어 복귀 결정 D8 — R11 릴리스 전
...
- [x] i18n ko/en parity 완성 — 잔여 main-process 한국어 고정 라벨(`notification.approvalPending.*` 등 R7/R8 deferred) `notification-labels.ts` D8 dictionary 이전 + R3~R9 의 ko 만 채워진 키 en 일괄 populate (orphan 0) + `setNotificationLocale` SettingsTabs.LanguageTab wire (현재 `DEFAULT_LOCALE='ko'` 고정 해소) + `i18next-parser` keepRemoved regex 확장 + `npm run i18n:check` idempotent (두 번째 실행 diff 0) — 산출물: `src/renderer/i18n/locales/{ko,en}.json` + `src/main/notifications/notification-labels.ts` + `i18next-parser.config.js`
- [x] Playwright Electron E2E OS matrix CI 활성화 (D5 GitHub Actions 표준 hosted runner, self-hosted 미도입) — Windows + macOS + Linux(xvfb-run headless) × 9 spec(external-project / messenger / meeting / approval / member-profile / autonomy-queue + R10 신규 search / dm / settings-tabs) = 27 matrix cell. R4~R9 내내 WSL DONE_WITH_CONCERNS 로 남겨둔 부채 일괄 해소. autonomy-queue-flow Step C (mock breaker 주입 후 강제 manual 다운그레이드) 활성화. API 키 없는 환경은 mock provider fixture reuse (R4~R9 패턴) — 산출물: `.github/workflows/playwright.yml` + `e2e/playwright.config.ts` projects matrix 확장 + `e2e/autonomy-queue-flow.spec.ts` Step C 활성
- [x] R10 Closeout — typecheck(전체) / typecheck:web / lint / test / i18n:check / theme:check / build / migration(012) / Playwright OS matrix exit 0 + R10 신규 250+ 테스트 green + R9 회귀 0 + 6 테마 12 스크린샷 사용자 sign-off + done-checklist 작성 + spec §10 R10 ✓ 전환 + tasks.json 15/15 completed — 산출물: `docs/checklists/r10-done-checklist.md`
...
**Phase R11 — 레거시 청소 + 릴리스** (plan: `docs/plans/2026-04-26-rolestra-phase-r11.md`, done-checklist: `docs/checklists/r11-done-checklist.md`)

R10 까지 "쓸 수 있는 v3" 가 닫힌 상태(15/15, design polish 라운드 1·2 commit 4건 포함, main tip `dc4a763`) 위에서, R11 은 (a) v2 레거시 코드 물리 삭제, (b) R10 Known Concerns 8건 일괄 종결, (c) Onboarding 첫 부팅 wizard 정식(step 3/4/5 + provider:detect + 영구화 + 자동 진입 — design polish 라운드 2 A 이월), (d) Approvals 상세 패널(design polish 라운드 2 B 이월), (e) Windows installer / macOS dmg / Linux AppImage 패키징 + GitHub Actions release workflow, (f) 사용자 문서(`docs/설계-문서.md`) v3 전면 재작성 — 6 축을 완성하여 첫 사용자 출시 phase 를 닫는다. 신규 도메인 모델 0 원칙 유지, 단 마이그레이션 2건(D3 `013_onboarding_state` + D4 `014_llm_cost_audit_log`) 만 예외. Decision Log D1~D9 은 `docs/plans/2026-04-26-rolestra-phase-r11.md` 참조.

구현 체크리스트 (완료 시 ✓ + 산출물 링크 채움):

- [x] §10 R11 본 정의 첫 항목 — `_legacy/migrations-v2/` (8 파일) + `_legacy/renderer-v1/` (8 디렉토리, 752KB) 일괄 물리 삭제(D1 단일 commit) + grep 검증(`_legacy` 인용 0) + typecheck/build/test 회귀 0 — 산출물: `_legacy/` 디렉토리 부재 (repo -752KB)
- [x] §10 R10 scope 경계 line 1327 R11 이연 — v2 engine 6 파일(`orchestrator.ts` / `turn-executor.ts` / `conversation.ts` / `execution-coordinator.ts` / `memory-coordinator.ts` / `persona-builder.ts`) + 대응 `__tests__/*.test.ts` 동시 삭제 + 7 legacy `@ts-nocheck` 파일(`consensus-driver.ts` / `consensus-handler.ts` / `permission-handler.ts` / `workspace-handler.ts` 등) 케이스 별 (a) 삭제 또는 (b) v3 재작성 + 주석 제거(D1 단계 분리 commit) — 산출물: `src/main/engine/` v2 파일 부재 + `grep -rln "@ts-nocheck" src/` 0 hit
- [x] R10 Known Concern #8 종결 — Pre-existing 14 test file 실패 정리(`database-*` 4 / `memory-*` 3 / `recovery-*` 2 / `remote-*` 3 / `handlers-v3 ipc` 2) — 각 파일 (i) v3 재작성 / (ii) `it.skip` + V4/R12+ reason / (iii) 물리 삭제 — `npm run test` exit 0 baseline 회복 — 산출물: 14 파일 처리 완료 + `npm run test` exit 0
- [x] R10 Known Concerns #1+#2+#3+#6 동시 종결 — Playwright OS matrix 안정화 + 누락 spec 2 작성(`e2e/search-flow.spec.ts` + `e2e/dm-flow.spec.ts`) + autonomy-queue Step C 활성(`__rolestraDevHooks.tripFilesPerTurn(21)` debug hook 을 preload contextBridge 에 `ROLESTRA_E2E=1` 가드로 노출) + workflow_dispatch 실 run 9 spec × 3 OS = 27 cell green + 12 스크린샷(6 테마 × 2 surface) Windows native + macOS native 캡처 — 산출물: `e2e/{search-flow,dm-flow}.spec.ts` + `src/preload/index.ts` (`__rolestraDevHooks` 가드) + `src/main/index.ts` (`ROLESTRA_E2E` 가드) + `e2e/autonomy-queue-flow.spec.ts` Step C 활성 + `.github/workflows/playwright.yml` 실 run + `docs/specs/appendix/r10-evidence/screenshots/`
- [x] §6 IPC R11 신규 8 채널 + Shared types — `onboarding:get-state`, `onboarding:set-state`, `onboarding:complete`, `provider:detect`, `llm:cost-summary`, `execution:dry-run-preview`, `approval:detail-fetch`, `meeting:voting-history` + zod round-trip 16+ + `'summarize'` capability literal 추가(Task 9 정식 사용) — 산출물: `src/shared/{onboarding-types,llm-cost-types,approval-detail-types,provider-types,ipc-types,ipc-schemas}.ts` + `src/preload/index.ts` 화이트리스트
- [x] §7.x Onboarding 첫 부팅 wizard 정식 — design polish 라운드 2 A(step 2 fixture) 위에 step 3(역할 부여) / step 4(권한 설정 — PermissionFlagBuilder reuse) / step 5(첫 프로젝트 생성) + `provider:detect` IPC 신설(capability snapshot) + 영구화(D3 신규 마이그레이션 013 `onboarding_state` 단일 row CHECK id=1) + 첫 부팅 자동 진입(NavRail 우회) + 완료 후 Dashboard 진입 + Settings.AboutTab "온보딩 다시 시작" CTA 정식 wire — 산출물: `src/main/database/migrations/013-onboarding-state.ts` + `src/main/onboarding/{onboarding-service,onboarding-state-repository}.ts` + `src/main/ipc/handlers/{onboarding-handler,provider-handler}.ts` + `src/renderer/features/onboarding/{OnboardingPage,use-onboarding-state,onboarding-data}.ts(x)` + `src/renderer/features/onboarding/steps/{Step3RoleAssignment,Step4Permissions,Step5FirstProject}.tsx` + `src/renderer/App.tsx` (first-boot view 강제) + `src/renderer/features/settings/tabs/AboutTab.tsx` (CTA wire) + `e2e/onboarding-flow.spec.ts`
- [x] §7.x Approvals 상세 패널(design polish 라운드 2 B 이월) — `ExecutionService.dryRunPreview` IPC 신설(read-only wrapper, atomic apply 안 함) + `approval:detail-fetch` 통합 IPC + `meeting:voting-history` IPC + `approval:list({status})` filter wiring(이미 IPC 받음, frontend hook) + `ApprovalDetailPanel` + 5 카드(Header / ImpactedFiles / DiffPreview / ConsensusContext / ActionBar) + ApprovalInboxView list+detail split layout — 시안 03 fidelity 도달 — 산출물: `src/main/execution/execution-service.ts` (`dryRunPreview`) + `src/main/ipc/handlers/{execution-handler,approval-handler,meeting-handler}.ts` + `src/renderer/features/approvals/use-approval-detail.ts` + `src/renderer/features/approvals/detail/{ApprovalDetailPanel,ApvDetailHeader,ApvImpactedFilesCard,ApvDiffPreviewCard,ApvConsensusContextCard,ApvActionBar}.tsx` + `src/renderer/features/approvals/{ApprovalInboxView,ApprovalFilterBar}.tsx` (split layout + onChange wiring) + `e2e/approval-detail-flow.spec.ts`
- [x] R10 D7 open question 첫 번째 항목 종결 — LLM 비용 가시화 — D4 신규 마이그레이션 014 `llm_cost_audit_log` append-only(`meeting_id` / `provider_id` / `token_in` / `token_out` / `created_at` + 2 인덱스) + `LlmCostRepository.append/summary/summaryByProvider` + `meeting-summary-service` 가 provider 응답 usage 객체(Anthropic `input_tokens/output_tokens`, OpenAI `prompt_tokens/completion_tokens`) 추출 후 append + `llm:cost-summary` IPC + Settings.AutonomyDefaultsTab "LLM 사용량" 섹션(D5 사용자 입력 단가 × 토큰, R11 default USD 0) — 산출물: `src/main/database/migrations/014-llm-cost-audit-log.ts` + `src/main/llm/{llm-cost-repository,meeting-summary-service}.ts` + `src/main/ipc/handlers/llm-handler.ts` + `src/renderer/hooks/use-llm-cost-summary.ts` + `src/renderer/features/settings/tabs/AutonomyDefaultsTab.tsx`
- [x] R10 Known Concern #7 종결 — `'summarize'` capability literal 정식 도입 + 6 provider config 갱신(Claude API/Codex/Gemini/Anthropic API direct/OpenAI API direct/Local Ollama) + `meeting-summary-service` 가 `'summarize'` true fallback chain(임시 `'streaming'` 우회 코드 0) — 산출물: `src/shared/provider-types.ts` (`ProviderCapability` union 확정) + `src/main/providers/registry.ts` + `src/main/llm/meeting-summary-service.ts`
- [x] R10 Known Concern #4 종결 — mode_transition conditional advisory 자동 주입 — `ApprovalSystemMessageInjector` filter 완화(mode_transition 만 예외 — channel 안 가도 됨) + `ProjectService.setPendingAdvisory/consumePendingAdvisory` (D7 in-memory only, 다음 회의까지만) + `MeetingOrchestrator.start()` 직후 advisory 소비 + 소비 후 slot 비움(이중 주입 방지) — 산출물: `src/main/projects/project-service.ts` + `src/main/approvals/{approval-decision-router,approval-system-message-injector}.ts` + `src/main/meetings/engine/meeting-orchestrator.ts`
- [x] R10 Known Concern #5 종결 + R11 신규 namespace populate — i18n ko/en parity 확장 — `onboarding.step3/4/5.*` + `approval.detail.*` + `llm.cost.*` (49+ 신규 키) + main-process 잔여 한국어 trace 라인(`approval-notification-bridge` / `autonomy-gate` 등) D9 결정(default = 한국어 유지 + locale 분기 옵션) 적용 — `notification-labels.ts` dictionary 이전 — `npm run i18n:check` exit 0 idempotent — 산출물: `src/renderer/i18n/locales/{ko,en}.json` + `src/main/notifications/notification-labels.ts` + `src/main/approvals/approval-notification-bridge.ts` + `src/main/autonomy/autonomy-gate.ts` + `i18next-parser.config.js`
- [x] §10 R11 본 정의 두 번째 항목 — 패키징 정식(D2 electron-builder 채택) — Windows NSIS installer + macOS dmg unsigned(R12+ 코드사인) + Linux AppImage(x64) + `npm run package:{win,mac,linux}` + `npm run package`(current OS) + `.github/workflows/release.yml` (tag push trigger 3 OS matrix + artifact upload + GitHub Release draft) + 로컬 검증(`npm run package:linux` WSL native) + 앱 아이콘(`assets/icon.{ico,icns,png}`) + Gatekeeper 우회 안내 docs — 산출물: `electron-builder.yml` + `package.json` scripts + `assets/icon.{ico,icns,png}` + `.github/workflows/release.yml`
- [x] §10 R11 본 정의 세 번째 항목 — 사용자 문서 v3 전면 재작성 — `docs/설계-문서.md` v2(Python/FastAPI/Svelte) → v3(Electron 40 + TS strict + React 19 + Tailwind + Radix + Zustand + better-sqlite3 + react-i18next + Vitest + Playwright + electron-builder) + `docs/기능-정의서.md` Rolestra v3 메타포 + 10 phase + 12 SSM state + 6 테마 + 자율 모드 4 tripwire + onboarding 반영(ko-only) + `docs/구현-현황.md` R1~R11 일괄 갱신 + ADR phase 별 묶음(D8 — `R1-R3-decisions.md` / `R4-R6-decisions.md` / `R7-R9-decisions.md` / `R10-decisions.md` / `R11-decisions.md` / `cross-cutting.md`) + `docs/design/` 신규(`Rolestra_sample/` 이전 + 12 스크린샷 sign-off 마크) + CLAUDE.md 문서 표 갱신 — 산출물: `docs/설계-문서.md` + `docs/기능-정의서.md` + `docs/구현-현황.md` + `docs/decisions/` (6 묶음 ADR) + `docs/design/` + `CLAUDE.md`
- [x] R10 D5 risk 종결 — CI macOS hosted runner 비용 monitoring — `.github/workflows/usage-report.yml` weekly cron(Sunday UTC) GitHub API billing fetch + `playwright.yml` PR trigger 분리(Windows + Linux 만, `[macos]` label 또는 manual workflow_dispatch 시만 macOS 추가, weekly cron Monday 풀 매트릭스) + README badge(선택) — 산출물: `.github/workflows/{usage-report,playwright}.yml`
- [x] R10 D8 결정 후속 — Optimistic UI 확장(`ApprovalBlock.decide` + `MemberProfile.edit`) — R10 의 3 hook 패턴(use-channel-messages.send / use-autonomy-mode.confirm / use-queue.addLines) reuse + 컴포넌트 내부 reducer만 변경 + 결정 직후 row status 임시 변경 + 실패 시 rollback + ErrorBoundary toast + dialog 재열림 + form state 복구 + 신규 i18n 키(`approval.decide.error.optimisticRollback` / `member.edit.error.optimisticRollback`) — 산출물: `src/renderer/features/messenger/ApprovalBlock.tsx` + `src/renderer/features/members/MemberProfileEditModal.tsx` + 단위 테스트
- [x] R11 Closeout — typecheck(전체) / typecheck:web / lint / test(R11 신규 + R10 baseline 회복 + R3~R10 회귀 0) / i18n:check(idempotent) / theme:check / build / `npm run package`(current OS) artifact 산출 / migration 013+014 idempotent + down 가능 / Playwright OS matrix workflow_dispatch 9 spec × 3 OS = 27 cell green / 12 스크린샷 사용자 sign-off / done-checklist 작성 + spec §10 R11 ✓ 전환 + tasks.json 17/17 completed — 산출물: `docs/checklists/r11-done-checklist.md`
...
- 음성 메모 — V4
- 플러그인 시스템 — V4
- ComfyUI / SD 연동 (메모리 `rolestra-idea-comfyui-sd.md`, R11 종료 후 재검토) — V4
- 회의별 LLM 요약 provider drop-down (R10 D7 두 번째 항목) — V4
- MessageSearchView 사이드 패널 레이아웃 (R10 D2) — V4
- Onboarding 미캡처 step 3/4/5 페이지 풍부화 (시안 06 추가 캡처 대기) — V4
- design polish 라운드 3+ — Hero strip 통합 (G5) / InsightStrip footer 변경 (G4) / Queue 6-column 테이블 (Q3) — V4
- macOS 코드 사인 + 공증(notarize) — R12+
- Windows 코드 사인(EV cert) — R12+
- AutoUpdate (electron-updater) — R12+
- Remote Access v3 재설계 (R10 Known Concern #8 의 remote-* 테스트 skip 처리분) — R12+
- Memory Phase 3-b — 임베딩 + 하이브리드 검색 + 반성 + 진화 (Stanford 3-factor scoring) — R12+
- Sentry / 크래시 리포팅 (현재 로컬만) — R12+
- Localization 추가 (ja / zh-CN) — R12+
- Settings vertical sidebar nav (R10 D3 — horizontal Radix Tabs 유지) — 각하
- DM 별도 dm_sessions 테이블 (R10 D1 — channels.kind='dm' 유지) — 각하
...
- 실측 매트릭스 (Phase R1): `docs/specs/appendix/cli-matrix.md`
