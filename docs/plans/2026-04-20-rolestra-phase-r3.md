...
- Spec: `docs/specs/2026-04-18-rolestra-design.md` §7.2, §7.5, §7.10, §10
...
- 포맷 참고: `docs/plans/2026-04-18-rolestra-phase-r1.md`, `docs/plans/2026-04-18-rolestra-phase-r1.md.tasks.json`
...
- Modify: `docs/specs/2026-04-18-rolestra-design.md`
...
- Modify: `docs/specs/2026-04-18-rolestra-design.md`의 §13 또는 부록으로 **시안 → TS 매핑 테이블** 1개 추가(선택). theme-tokens.jsx 키 ↔ theme-tokens.ts 타입 ↔ CSS variable 이름 3열 표.
...
- `git diff docs/specs/2026-04-18-rolestra-design.md` 리뷰로 §7.2/§7.5/§7.10/§10 네 곳 변경 확인
...
**Goal:** R2 `router.ts`의 `LEGACY_V2_CHANNELS`이 warn을 내는 채널을 **현재 누가 부르는가** 조사해 `docs/specs/appendix/legacy-channels.md`에 표로 기록. R3는 **경고 유지, 제거 안 함**(제거는 R11). 새 렌더러는 legacy 채널 호출 0건이어야 함 — 이를 테스트로 고정.

**Files:**

- Create: `docs/specs/appendix/legacy-channels.md` — 표 3열: `채널명 / 현재 호출 위치 / R11 제거 시 마이그레이션 계획`. 현재 호출 위치 컬럼에는 `_legacy/renderer-v1/` 하위 파일 path만 오게 된다.
...
- `diff <(grep -oE "'[a-z]+:[a-z-]+'" src/main/ipc/router.ts | sort -u) <(grep -oE "'[a-z]+:[a-z-]+'" docs/specs/appendix/legacy-channels.md | sort -u)` 에서 legacy prefix 일치(완전 일치가 아니더라도 5 prefix 전부 포함)
...
- Create: `docs/specs/appendix/r3-evidence/` — 6 테마 × 2 모드 = 6 PNG(Playwright MCP 또는 수동 dev 캡처). 파일명 `warm-light.png` 등.
- Modify: `docs/specs/2026-04-18-rolestra-design.md` §10 R3 항목 — 체크박스 전부 ✓ + 산출물 링크(`_legacy/renderer-v1/`, `src/renderer/theme/`, `src/renderer/components/shell/`, `src/renderer/styles/tokens.css`, `docs/specs/appendix/legacy-channels.md`, `appendix-r3-evidence/`)
- Create: `docs/checklists/r3-done-checklist.md` — 완료 확인 항목 및 R4 착수 조건
...
- `ls docs/specs/appendix/r3-evidence/ | wc -l` ≥ 6
