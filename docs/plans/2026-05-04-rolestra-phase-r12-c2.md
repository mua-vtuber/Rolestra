...
- `MeetingOrchestrator` 새 모델 재배선 (12 단계 SSM 폐기) — **T10 sub-task 분할** (2026-05-04 사용자 결정): T10a (새 backend skeleton + 새 IPC) → T10b (옛 SSM + 옛 IPC + 옛 SsmBox 정리) → T10c (e2e + closeout). 풀세트 3 부서 (기획 / 리뷰 / 검토) backend 토대만 — 아이디어 = D-B-Light (P3), 디자인 = 7 단계 (P3).
- *T10 의 옛 ProjectEntryView reverted 의 재구현* — 할 일 큐 textarea → 부서 회의 자동 트리거 + 부서 채널 입력란 enable 분기 wire (T11 disabled 토대 위, T10c land 후 진입)
...
|-------|-------------|------|--------------|
| P2 회의 backend | T7~T12 + T36~T37 | 8 | migration 019 / OpinionService / MeetingMinutesService / MeetingOrchestrator 재배선 (T10a/b/c 분할 — 2026-05-04 사용자 결정) / 할 일 큐 트리거 / 회의록 chat block |
