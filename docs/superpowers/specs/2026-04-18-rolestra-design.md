...
- [ ] §6 IPC autonomy + queue + notification prefs 확정 — `project:set-autonomy(id, mode)` + `queue:list/add/reorder/remove/pause/resume` + `notification:get-prefs/update-prefs/test` + Shared autonomy/queue/notification types + zod — 산출물: `src/shared/{queue-types,notification-types,ipc-types,ipc-schemas}.ts` + `src/preload/index.ts`
- [ ] §7.3 `AutonomyModeToggle` (프로젝트 헤더, manual/auto_toggle/queue 3분기) + 2단계 확인 다이얼로그 (manual → auto_toggle/queue 승격 시 Circuit Breaker 기본값 안내 + 명시적 재확인) + `use-autonomy-mode` hook + `stream:autonomy-mode-changed` reducer — 산출물: `src/renderer/features/projects/AutonomyModeToggle.tsx` + `src/renderer/features/projects/AutonomyConfirmDialog.tsx` + `src/renderer/hooks/use-autonomy-mode.ts`
- [ ] §8 `QueuePanel` (프로젝트 상단 접기 가능 패널, 여러 줄 입력 → 항목 추가 / 목록 / drag 순서 변경 / 항목 제거 / pause / resume / 진행 포인터 표시) + `use-queue` hook + `stream:queue-updated` reducer — 산출물: `src/renderer/features/projects/QueuePanel.tsx` + `src/renderer/hooks/use-queue.ts`
- [ ] §7.8 `NotificationPrefsView` (설정 UI — 4 kind `new_message`/`approval_pending`/`work_done`/`error` × 표시 on/off + 소리 on/off + 테스트 알림 버튼) + `use-notification-prefs` hook — 산출물: `src/renderer/features/settings/NotificationPrefsView.tsx` + `src/renderer/hooks/use-notification-prefs.ts` + 설정 surface 진입점
- [ ] §8 `AutonomyGate` — `ApprovalService.create` 시 autonomyMode 분기 (manual=approval_item, auto_toggle/queue=정책 조건 accepted kind 만 auto accept + log + `#회의록` trace) — 산출물: `src/main/autonomy/autonomy-gate.ts` + `src/main/approvals/approval-service.ts` 후킹 + `src/main/autonomy/__tests__/autonomy-gate.test.ts`
- [ ] §8 Circuit Breaker 실 이벤트 feed (4 tripwire: `files_per_turn`/`cumulative_cli_ms`/`queue_streak`/`same_error`) — `ExecutionService.dryRunApply` 직후 `recordFileChanges`, `CliRunner.spawn` 완료 시 `recordCliElapsed`, `QueueService.startNext` 시 `recordQueueStart`, `MeetingTurnExecutor`/`ApprovalService` 실패 시 `recordError` + `on('fired')` → `ProjectService.setAutonomy('manual')` + `approval_item` + `NotificationService.show('error', { reason })` — 산출물: `src/main/engine/v3-side-effects.ts` (breaker handler 완성) + `src/main/execution/execution-service.ts` + `src/main/providers/cli-runner.ts` + `src/main/queue/queue-service.ts` + `src/main/meetings/engine/meeting-turn-executor.ts` hooks
- [ ] §8 Queue run loop — QueueService auto-start 다음 항목 (이전 meeting onFinal 직후) + 앱 재시작 복구 (`status='in_progress'` → `pending` 롤백 + `attempts` 유지) + pause/resume 정합성 + `stream:queue-updated` — 산출물: `src/main/queue/queue-service.ts` 확장 + `src/main/queue/__tests__/queue-service.test.ts` 확장 + `src/main/index.ts` recovery hook
- [ ] §8 `MeetingOrchestrator.onFinal` → `#회의록` 요약 메시지 자동 포스트 + `NotificationService.show('work_done', { projectId, meetingTitle })` (autonomyMode 무관 공통) + `#일반` 완료 알림(auto_toggle/queue 모드 한정, spec §8) — 산출물: `src/main/engine/v3-side-effects.ts` work-done handler 완성 + `src/main/meetings/engine/meeting-orchestrator.ts` wire
- [ ] §7.8 Production wiring — `setNotificationServiceAccessor` + `setQueueServiceAccessor` + `streamBridge.connect({ ..., queue: queueService })` + `NotificationPrefsRepository` 기본값 seed + `notification-handler` / `queue-handler` production 활성화 (R8 Task 8 패턴 — 테스트에서만 wire 되어 있던 6 IPC 를 production 에서 throw 0 으로) — 산출물: `src/main/index.ts` R9 boot block
- [ ] R8 인수인계 통합: (a) 외근 자동 timeout (1h 지나면 `offline-manual` → `online` 자동 복귀; 정책 값은 settings 에서 조정 가능; 기본 60분) + (b) `MemberWarmupService` backoff retry (실패 시 10s → 30s → 60s 지수, 최대 3회) — 산출물: `src/main/members/member-profile-service.ts` timeout 관리 + `src/main/members/member-warmup-service.ts` retry 확장 + tests
- [ ] Stream 브로드캐스트 `stream:queue-updated` / `stream:notification-prefs-changed` / `stream:autonomy-mode-changed` — QueueService / NotificationService / ProjectService event emit → StreamBridge → preload 화이트리스트 → renderer hooks reducer — 산출물: `src/shared/stream-events.ts` + `src/main/streams/stream-bridge.ts` + `src/preload/index.ts` + `src/renderer/hooks/{use-queue,use-notification-prefs,use-autonomy-mode}.ts`
- [ ] i18n populate `notification.*` (main-process 라벨 — R7/R8 에서 deferred 됐던 4 kind 고정 title/body + `notification.warmupFailed.*` 등) / `autonomy.*` (mode 3 라벨 + confirm dialog + Circuit Breaker 발동 안내) / `queue.*` (panel + toast + recovery banner) / `circuitBreaker.*` (4 tripwire 라벨 + reason 설명) / `settings.notifications.*` + i18next-parser keepRemoved 확장 — 산출물: `src/renderer/i18n/locales/{ko,en}.json` + `i18next-parser.config.js`
- [ ] §11 Playwright Electron E2E "auto_toggle 승격 2단계 확인 → 모드 전환 자동 accept → queue 추가/시작 → 완료 시 `#일반` 알림 + 다음 항목 자동 시작 → Circuit Breaker `files_per_turn > 20` 시 강제 manual 다운그레이드 + OS 알림" — 산출물: `e2e/autonomy-queue-flow.spec.ts` (mock CLI + mock ExecutionService, WSL 제약 시 R4~R8 와 동일 DONE_WITH_CONCERNS 정책)
- [ ] typecheck(전체) / typecheck:web / lint / test / i18n:check / theme:check / build exit 0 + R9 신규 테스트 green + done-checklist 작성 — 산출물: `docs/superpowers/specs/r9-done-checklist.md`
