...
- [ ] §10 R11 본 정의 두 번째 항목 — 패키징 정식(D2 electron-builder 채택) — Windows NSIS installer + macOS dmg unsigned(R12+ 코드사인) + Linux AppImage(x64) + `npm run package:{win,mac,linux}` + `npm run package`(current OS) + `.github/workflows/release.yml` (tag push trigger 3 OS matrix + artifact upload + GitHub Release draft) + 로컬 검증(`npm run package:linux` WSL native) + 앱 아이콘(`assets/icon.{ico,icns,png}`) + Gatekeeper 우회 안내 docs — 산출물: `electron-builder.yml` + `package.json` scripts + `assets/icon.{ico,icns,png}` + `.github/workflows/release.yml`
