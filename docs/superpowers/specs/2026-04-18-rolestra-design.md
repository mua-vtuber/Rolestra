...
- [ ] §10 R11 본 정의 첫 항목 — `_legacy/migrations-v2/` (8 파일) + `_legacy/renderer-v1/` (8 디렉토리, 752KB) 일괄 물리 삭제(D1 단일 commit) + grep 검증(`_legacy` 인용 0) + typecheck/build/test 회귀 0 — 산출물: `_legacy/` 디렉토리 부재 (repo -752KB)
- [ ] §10 R10 scope 경계 line 1327 R11 이연 — v2 engine 6 파일(`orchestrator.ts` / `turn-executor.ts` / `conversation.ts` / `execution-coordinator.ts` / `memory-coordinator.ts` / `persona-builder.ts`) + 대응 `__tests__/*.test.ts` 동시 삭제 + 7 legacy `@ts-nocheck` 파일(`consensus-driver.ts` / `consensus-handler.ts` / `permission-handler.ts` / `workspace-handler.ts` 등) 케이스 별 (a) 삭제 또는 (b) v3 재작성 + 주석 제거(D1 단계 분리 commit) — 산출물: `src/main/engine/` v2 파일 부재 + `grep -rln "@ts-nocheck" src/` 0 hit
- [ ] R10 Known Concern #8 종결 — Pre-existing 14 test file 실패 정리(`database-*` 4 / `memory-*` 3 / `recovery-*` 2 / `remote-*` 3 / `handlers-v3 ipc` 2) — 각 파일 (i) v3 재작성 / (ii) `it.skip` + V4/R12+ reason / (iii) 물리 삭제 — `npm run test` exit 0 baseline 회복 — 산출물: 14 파일 처리 완료 + `npm run test` exit 0
- [ ] R10 Known Concerns #1+#2+#3+#6 동시 종결 — Playwright OS matrix 안정화 + 누락 spec 2 작성(`e2e/search-flow.spec.ts` + `e2e/dm-flow.spec.ts`) + autonomy-queue Step C 활성(`__rolestraDevHooks.tripFilesPerTurn(21)` debug hook 을 preload contextBridge 에 `ROLESTRA_E2E=1` 가드로 노출) + workflow_dispatch 실 run 9 spec × 3 OS = 27 cell green + 12 스크린샷(6 테마 × 2 surface) Windows native + macOS native 캡처 — 산출물: `e2e/{search-flow,dm-flow}.spec.ts` + `src/preload/index.ts` (`__rolestraDevHooks` 가드) + `src/main/index.ts` (`ROLESTRA_E2E` 가드) + `e2e/autonomy-queue-flow.spec.ts` Step C 활성 + `.github/workflows/playwright.yml` 실 run + `docs/superpowers/specs/appendix-r10-evidence/screenshots/`
