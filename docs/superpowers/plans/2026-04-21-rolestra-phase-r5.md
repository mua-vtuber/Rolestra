...
- Spec:
  - `docs/superpowers/specs/2026-04-18-rolestra-design.md` §5.1, §5.2(channels / channel_members / messages / messages_fts), §6(channel:* + message:* + meeting:*), §7.4(채널 시스템), §7.10(디자인 시스템), §10 Phase R5
- Prep:
  - `docs/superpowers/specs/r5-prep-messenger-theme-analysis.md` — 본 plan은 이 문서 §4 인벤토리 + §5 토큰 결정 + §6 필수 AC 항목을 **그대로** 반영한다. 보류 결정 4건(§8)은 Task 0에서 사용자 confirm 후 확정.
- R4 plan: `docs/superpowers/plans/2026-04-20-rolestra-phase-r4.md` — Radix Dialog 모달 패턴, renderer hook 패턴, IPC wrapper 확장 패턴 답습.
- 시안 정본:
...
- [ ] `docs/superpowers/plans/2026-04-21-rolestra-phase-r5.md.tasks.json` 생성(R4 포맷 답습, 14 task slot, 본 Task부터 `completed=true` 체크)
...
- [ ] `docs/superpowers/specs/r5-done-checklist.md` 작성(R4 템플릿 복제):
...
- [ ] `docs/superpowers/plans/2026-04-21-rolestra-phase-r5.md.tasks.json` 모든 태스크 `completed: true`
